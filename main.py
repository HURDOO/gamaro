...
def compress(img, source_size, destination_size, step):
    ...
    for i in range(i_count):
        ...
        inputs.append((i, i_count, j_count, img, destination_size, transformed_blocks))
    pool = multiprocessing.Pool(processes=16)
    results = pool.starmap(compress_sub, inputs)

    return results


def compress_sub(i, i_count, j_count, img, destination_size, transformed_blocks):
    res = []
    for j in range(j_count):
        ...
        for k, l, direction, angle, S in transformed_blocks:
            ...
            if d < min_d:
                min_d = d
                res[j] = (k, l, direction, angle, contrast, brightness)
    return res
...
def test_greyscale():
    ...
    img = Image.open('cat-muzzle-spotted-look-wallpaper-preview (1).jpg')
    img = img.convert('RGB')
    img.save('work.gif')

    img = mpimg.imread('work.gif')
    ...
    iterations = decompress(transformations, 8, 4, 8, nb_iter=8)
    # plt.imshow(iterations[-1], cmap='gray', interpolation='none')
    plot_iterations(iterations, img)
    plt.show()


def test_rgb():
    img = mpimg.imread('cat-muzzle-spotted-look-wallpaper-preview (1).jpg')
...
def test_restore():
    ...
    # print(img)
    # img = img.reshape((img.shape[0]*2, img.shape[1]*2))
    ...
    for i in range(1, 31, 2):
        retrieved_img = decompress_rgb(transformations, 8, 4, 8, nb_iter=i)
...
def test_restore_greyscale():
    ...
    transformations = compress(img, 8, 4, 8)
    iterations = decompress(transformations, 8, 4, 8, nb_iter=7)
    plot_iterations(iterations, img)

    Image.fromarray(iterations[-1]).save('result.gif')
